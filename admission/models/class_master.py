...
class ClassMaster(models.Model):
    ...
    line_ids = fields.One2many('student.lines', 'line_id', string='Fee Details')
    name = fields.Char(string="Class Room", index=True, required="1")
    ...
    cordinator_id = fields.Many2one('res.users', string="Academic Cordinator", default=lambda self: self.env.user.id,
                                    readonly="1")
    approve_id = fields.Many2one('res.users', string="Approved By", default=lambda self: self.env.user.id, readonly="1", tracking= True)
    ...
    @api.depends('total_seats', 'line_ids')
    def _compute_seats(self):
        # num = 0
        for k in self:
            k.available_seats = k.total_seats - len(k.line_ids)

    @api.depends('line_ids.pending_fee')
    ...
    def action_reallocate(self):
        ...
        return {
            'name': _('Reallocation'),
            'view_mode': 'form',
            'res_model': 'classroom.reallocate.student',
            'type': 'ir.actions.act_window',
            'target': 'new',
            "context": {
                'default_class_id': self.id,
...
class StudentLines(models.Model):
    ...
    line_id = fields.Many2one('res.class')
    student_id = fields.Many2one('res.partner', string="Student")
    batch_id = fields.Many2one('res.batch',string="Batch")
    pending_fee = fields.Float(string='Pending Fee')
    ad_id= fields.Many2one('res.admission',string='Admision Number')
    state = fields.Selection([
        ('draft', 'Draft'),
        ('confirm', 'Confirmed'),
        ('transfer', 'Transfer'), ('cancel', 'Cancel'), ('drop', 'Dropped')],related='ad_id.state',  default='draft', string='Status',
        store=True)
